...
describe("Default Date Util", () => {
    ...
    describe("format string by formatter", () => {
        it("should format iso string", () => {
            const result = defaultDateUtil.format("2011-02-01T01:02:03", "dd.MM.yyyy HH:mm:ss");
            expect(result).toBe("01.02.2011 01:02:03");
        });

        it("should format date", () => {
            const result = defaultDateUtil.format(new Date(2011, 1, 1, 1, 2, 3), "dd.MM.yyyy HH:mm:ss");
            expect(result).toBe("01.02.2011 01:02:03");
        });

        it("should format date without time", () => {
            const result = defaultDateUtil.format(new Date(2011, 1, 1), "dd.MM.yyyy HH:mm:ss");
    ...
    describe("Set", () => {
        const cases: [DateUnion, SetDateValues, DateUnion][] = [
            [new Date(2020, 0, 1), { hours: 1, minutes: 2, seconds: 3 }, new Date(2020, 0, 1, 1, 2, 3)],
            [
                "2020-01-01T00:00:00",
                {
                    hours: 1,
