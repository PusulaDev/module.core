...
describe("Core Mapper", () => {
    ...
    it("should map to target array and source array", () => {
        ...
        const mapped = mapper.mapToTargetList([{ name: "first" }, { name: "second" }]);

        const mappedSource = mapper.mapToSourceList([{ name: "first" }, { name: "second" }]);
    ...
    it("should map to source", () => {
        ...
        mapper
            .forSource("count", (target) => target.list.reduce((count: number, e) => e + count, 0))
