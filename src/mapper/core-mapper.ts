import type { IMapper, MapperConfiguration, MapperTargetValue } from "./mapper.interface";
...
export class CoreMapper<Source, Target> implements IMapper<Source, Target> {
    ...
    setTargetConfig(config: Omit<MapperConfiguration<Source, Target>, "fieldConfigurations">) {
        this.targetConfiguration = { ...this.targetConfiguration, ...config };
    }

    setSourceConfig(config: Omit<MapperConfiguration<Target, Source>, "fieldConfigurations">) {
    ...
    forTarget<TargetField extends keyof Target>(
        ...
        this.targetConfiguration.fieldConfigurations[targetField] = sourceValue ?? (targetField as any);
    ...
    forSource<SourceField extends keyof Source>(
        ...
        // eslint-disable-next-line @typescript-eslint/no-unsafe-assignment
        this.sourceConfiguration.fieldConfigurations[sourceField] = targetValue ?? (sourceField as any);
    ...
    private mapByFieldConfig<TSource, TTarget>(options: {
        ...
        for (const targetKey in options.configuration.fieldConfigurations) {
            const config = options.configuration.fieldConfigurations[targetKey as keyof TTarget];
